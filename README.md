...
- **Save-ChromeWindowTabs.ps1** - List visible Chrome windows, export tab URLs from a selected window, and restore a new window from a saved tabs file
- **Extract-TickersFromCsv.ps1** - Pick a CSV-formatted file, keep only the Ticker column, and save it as <input_name>_tickers_only<input_ext>
...
### Save-ChromeWindowTabs.ps1
Lists visible Chrome windows with a running ID, detected window name, and RAM usage. Exports all tab URLs from a selected window into `Chrome-Window-<window_name>-tabs.txt`. Also supports restoring tabs from a saved file into a new Chrome window.

### Extract-TickersFromCsv.ps1
Opens a file picker to select a CSV-formatted file, keeps only the `Ticker` column, and writes the result to `<input_name>_tickers_only<input_ext>` in the same folder.
...
- **Chrome Window Discovery**: Finds all visible Chrome windows and assigns each a running ID
- **Window Details**: Displays detected window name, full window title, and RAM usage
- **Interactive Selection**: User selects the target window by running ID
- **Tab URL Export**: Saves URLs to `Chrome-Window-<window_name>-tabs.txt`
- **Restore Mode**: Opens a new Chrome window from a previously exported tabs file

### Extract-TickersFromCsv.ps1

- **File Picker Input**: Opens a GUI file picker dialog for selecting the source file (CSV-formatted content)
- **Ticker-Only Output**: Removes all columns except `Ticker`
- **Consistent Output Name**: Saves output as `<input_name>_tickers_only<input_ext>`
- **Quote-Free Output**: Removes `"` characters from the output file
- **Validation and Errors**: Handles cancel action, missing files, missing `Ticker` column, and read/write issues
- **Optional Direct Path Mode**: Supports `-CsvPath` for non-interactive runs
...
# 2) In the prompt, choose a window by running ID (example)
Enter the running ID of the Chrome window to export: 2

# 3) Result file example
# .\Chrome-Window-Work-tabs.txt
```

The script will:
1. Find all visible Chrome windows
2. Show a list of windows with running ID, detected window name, RAM usage, and title
3. Ask you to select a window by running ID
4. Capture tab URLs from that window
5. Save URLs to `Chrome-Window-<window_name>-tabs.txt`

Run restore mode (next phase):

```powershell
.\Save-ChromeWindowTabs.ps1 -RestoreFromFile ".\Chrome-Window-Work-tabs.txt"
```

How to run restore (example):

```powershell
# Open a new Chrome window with tabs from a saved file
.\Save-ChromeWindowTabs.ps1 -RestoreFromFile ".\Chrome-Window-Work-tabs.txt"
```

The script will:
1. Load URLs from the specified tabs file
2. Start a new Chrome window
3. Open each URL as a tab
...
# Pick src.csv (or any CSV-formatted file) in the file picker
# Result file is created next to it:
# src_tickers_only.csv
```

Optional direct path mode:

```powershell
.\Extract-TickersFromCsv.ps1 -CsvPath ".\src.csv"
```

The script will:
1. Read the selected CSV
2. Verify that a `Ticker` column exists
3. Export a new CSV with only the `Ticker` column
4. Save output to `<input_name>_tickers_only<input_ext>` in the same folder
...
### Save-ChromeWindowTabs.ps1
- The script uses visible Chrome window titles to detect the window name; if Chrome provides only a page title, the detected name may reflect that title
- Tab capture temporarily focuses the selected Chrome window and cycles through its tabs
- RAM is reported as the working set of the owning `chrome.exe` process for each visible window

### Extract-TickersFromCsv.ps1
- The script looks for the `Ticker` column name case-insensitively
- If the input CSV has no data rows, the output file is created with only the `Ticker` header
- Output file is named `<input_name>_tickers_only<input_ext>` and is saved next to the selected source file
- The output file contains no `"` characters
...
4. **Save tabs from a Chrome window**:
  ```powershell
  .\Save-ChromeWindowTabs.ps1
  # Select a running ID when prompted
  ```
